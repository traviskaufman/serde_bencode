...
impl<R> Deserializer<R> where R: Read {
    ...
    fn parse_next<V>(&mut self, mut visitor: V) -> Result<V::Value> where V: de::Visitor {
    ...
    fn parse_int<V>(&mut self, mut visitor: V) -> Result<V::Value> where V: de::Visitor {
        const END: u8 = b'e';
...
impl<R> de::Deserializer for Deserializer<R> where R: Read {
    ...
    #[inline]
    fn deserialize<V>(&mut self, visitor: V) -> Result<V::Value> where V: de::Visitor {
...
impl<'a, R: Read + 'a> de::MapVisitor for MapVisitor<'a, R> {
    ...
    fn visit_key<K>(&mut self) -> Result<Option<K>> where K: de::Deserialize {
        const END: u8 = b'e';
    ...
    fn visit_value<V>(&mut self) -> Result<V> where V: de::Deserialize {
...
impl<'a, R: Read + 'a> de::SeqVisitor for SeqVisitor<'a, R> {
    ...
    fn visit<V>(&mut self) -> Result<Option<V>> where V: de::Deserialize {
        const END: u8 = b'e';
        match self.de.peek_char() {
            Some(END) => Ok(None),
...
fn from_read<R, T>(read: R) -> Result<T> where R: Read, T: de::Deserialize {
...
pub fn from_reader<R, T>(reader: R) -> Result<T> where R: io::Read, T: de::Deserialize {
    from_iter(reader.bytes())
}

pub fn from_slice<T>(s: &[u8]) -> Result<T> where T: de::Deserialize {
    from_read(read::SliceRead::new(s))
}

pub fn from_string<T>(s: String) -> Result<T> where T: de::Deserialize {
